...
#    2.0  [KD]  2020/12/02    added suport for setting alarm thresholds      #
#                                                                            #
#  Objective:                                                                #
...
#    Kevin DeTerra (added thresold functionality)                            #
#    mail: kdeterra@extremenetworks.com                                      #
##############################################################################
...
# DETERRAK
def getThresholds(thresholdsFile):
    # open a json file, returns a json object
    try:
        thresholds = open(thresholdsFile, 'rb').read()
 
        if thresholds:
            thresholds_json = json.loads(thresholds)
            return thresholds_json
    except:
        print("TWAMP INFO: could not load file %s. Using defaults instead." % thresholdsFile)
        return '{"default": {"delay": 10, "jitter": 5, "loss": 3} }'
    return False

def create_rotating_log(path, msg):
    """
    Creates a rotating log
    """
    logger = logging.getLogger("Rotating Log")
    logger.setLevel(logging.INFO)
    print("path=%s msg=%s" %(path, msg)) 
    print("adding a rotating handler")
    handler = RotatingFileHandler(path, maxBytes=2560, backupCount=5)
...
class udpSession(threading.Thread):
    ...
    def bind(self, addr, port, tos, ttl, df):
        ...
        # DETERRAK - Modifications for EXOS VR == Default-VR
        try:
            vrs = open('/proc/net/vr', 'r').read()
            #print(vrs)
            vrs.close()
        except:
            #print("Could not open file '/proc/net/vr'")
            pass

        print("Setting socket options to use the Default-VR\n")
        try:
            f = open('/proc/self/ns_id', 'w')
            f.write('2\n')
            f.close()
        except:
            #print("Could not find the file '/proc/self/ns_id'")
            pass
        ...
        if df:
            if (sys.platform == "linux2"):
                ...
                print("platform is not detected\n")
        else:
            if (sys.platform == "linux2"):
...
class twampStatistics():
    ...
    def dump(self, total, remote_addr):
        print("===============================================================================")
        print("Direction         Min         Max         Avg          Jitter     Loss")
        print("-------------------------------------------------------------------------------")
        if self.count > 0:
            self.lossRT = total - self.count
            print("Outbound:    %s  %s  %s  %s    %5.1f pkts" % (dp(self.minOB), dp(self.maxOB), dp(self.sumOB / self.count), dp(self.jitterOB), abs(100 * float(self.lossOB) / total)))
            print("Inbound:     %s  %s  %s  %s    %5.1f pkts" % (dp(self.minIB), dp(self.maxIB), dp(self.sumIB / self.count), dp(self.jitterIB), 100 * float(self.lossIB) / total))
            print("Roundtrip:   %s  %s  %s  %s    %5.1f pkts %5.1f%%" % (dp(self.minRT), dp(self.maxRT), dp(self.sumRT / self.count), dp(self.jitterRT), self.lossRT ,100 * float(self.lossRT) / total))
        else:
        ...
        #DETERRAK - Load Thresholds 
        thresholds = getThresholds("/config/twamp_thresholds.pol")
        if remote_addr in thresholds:
            target = remote_addr
        else:
            target = "default"

        # Thresholds
        IBD_th = thresholds[target]['IB_delay']
        OBD_th = thresholds[target]['OB_delay']
        RTD_th = thresholds[target]['RT_delay']
        IBJ_th = thresholds[target]['IB_jitter']
        OBJ_th = thresholds[target]['OB_jitter']
        RTJ_th = thresholds[target]['RT_jitter']
        RTL_th = thresholds[target]['RT_loss']
        print("Threshold target %s %s" % (target, thresholds[target]))

        # Measured Results
        OBD_av = (self.sumOB / self.count)
        IBD_av = (self.sumIB / self.count)
        RTD_av = (self.sumRT / self.count)


        # create a JSON object
        json_data = {}

        # DELAY threshold evaluation
        if IBD_av > IBD_th:
            json_data['IBD'] = dp(IBD_av).strip()
            print("The Average inbound delay %s exceeds the threshold %s" % (dp(IBD_av).strip(), dp(IBD_th).strip()))

        if OBD_av > OBD_th:
            json_data['OBD'] = dp(OBD_av).strip()
            print("The Average outbound delay %s exceeds the threshold %s" % (dp(OBD_av).strip(), dp(OBD_th).strip()))

        if RTD_av > RTD_th:
            json_data['RTD'] = dp(RTD_av).strip()
            print("The Average round-trip delay %s exceeds the threshold %s" % (dp(RTD_av).strip(), dp(RTD_th).strip()))

        # JITTER threshold evaluation
        if self.jitterIB > IBJ_th:
            json_data['IBJ'] = dp(IBJ).strip()
            print("The Average inbound jitter %s exceeds the threshold %s" % (dp(self.jitterIB).strip(), dp(IBJ_th).strip()))

        if self.jitterOB > OBJ_th:
            json_data['OBJ'] = dp(OBJ).strip()
            print("The Average outbound jitter %s exceeds the threshold %s" % (dp(self.jitterOB).strip(), dp(OBJ_th).strip()))

        if self.jitterRT > RTJ_th:
            json_data['RTJ'] = dp(RTJ).strip()
            print("The Average round-trip jitter %s exceeds the threshold %s" % (dp(self.jitterRT).strip(), dp(RTJ_th).strip()))

        # LOSS RT threshold evaluation
        loss = (100 * float(self.lossRT) / total)
        if loss > RTL_th:
            json_data['RTL'] = dp(loss).strip()
            print("The Average round-trip loss %5.1f%% exceeds the threshold %d" % (loss, RTL_th))

        # check to see if any key/value pairs were added
        if len(json_data) > 0:
            #date = datetime.now()
            json_data['TS'] = datetime.now().strftime("%m%d%Y,%H:%M:%S")
            json_data['DST'] = remote_addr
            print("The threshold exceeded: %s" % (json.dumps(json_data)))

            # escape the quotes in the JSON string
            msg = json.dumps(json_data).replace('"', r'\"')
            # write to the system log
            print("msg: %s" % msg)
            # Note: The max length of a syslog message (via EXOS CLI) is 128 bytes
            if  len(str(msg)) < 121:
                exsh.clicmd('create log message "TWAMP: %s"'  % msg )
            else:
                print("msg is too long to send via syslog")
            # send an snmp trap
            # Note: the max length of an SNMP trap (via EOXS) is 256 bytes
            if len(str(msg)) < 256:
                exsh.clicmd('create snmp trap severity notice event TWAMP "%s"' % msg)
            else:
                print("msg is too long to send via an snmp trap")

            # write to the 'twamp_buffer' log file
            try:
                log_name = '/config/twamp_buffer.pol'
                create_rotating_log(log_name, json.dumps(json_data))
            except:
                print("Error: could not write to the buffer!")

        sys.stdout.flush()

    def dumpSyslog(self, total):
        if self.count > 0:
            self.lossRT = total - self.count
            syslog.syslog("TWAMP OB, Min:%s,  Max:%s, Avg:%s, Jitter:%s, Loss:%5.1f%%" % (dp(self.minOB), dp(self.maxOB), dp(self.sumOB / self.count), dp(self.jitterOB), 100 * float(self.lossOB) / total))
            syslog.syslog("TWAMP IB, Min:%s, Max:%s, Avg:%s Jitter:%s, Loss:%5.1f%%" % (dp(self.minIB), dp(self.maxIB), dp(self.sumIB / self.count), dp(self.jitterIB), 100 * float(self.lossIB) / total))
            syslog.syslog("TWAMP RT, Min:%s, Max:%s, Avg:%s, Jitter:%s, Loss:%5.1f%%" % (dp(self.minRT), dp(self.maxRT), dp(self.sumRT / self.count), dp(self.jitterRT), 100 * float(self.lossRT) / total))
        else:
            syslog.syslog("TWAMP, NO STATS AVAILABLE (100% loss)")
        sys.stdout.flush()
...
class twampySessionSender(udpSession):
    ...
    def run(self):
        ...
        self.stats.dump(idx, self.remote_addr)

        #DETERRAK
        #self.stats.dumpSyslog(idx)
